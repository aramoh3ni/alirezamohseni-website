...
export const Projects = ({ data }) => {
    ...
    return (
        <>
            ...
            {
                data?.map(item => (
                    <Box key={item.id} borderWidth='1px' rounded='md' overflow='hidden' mb={2} p={3} className="card-shadow">
                        ...
                        {item.topics?.map(topic => (
                            <Badge p={1} borderRadius='md' ml={1} colorScheme='blue' variant='subtle'>
                                {topic}
                            </Badge>
                        ))}
